...
public class ControlsScript : MonoBehaviour {
	...
	)
    {
            ...
            )
        {
            ...
            if (!isBlocking && !blockStunned)
            {
                ...
                if (myMoveSetScript.IsAnimationPlaying("crouching") || myMoveSetScript.IsAnimationPlaying("crouching_2"))
                {
        ...
        if (myMoveSetScript.IsAnimationPlaying("idle")
            ...
            if (afkTimer >= myMoveSetScript.basicMoves.idle._restingClipInterval) {
                afkTimer = 0;
                //FPRandom newRandomSeed = FPRandom.New((int)UFE.currentFrame);
                //int clipNum = newRandomSeed.Next(2, 6);
                int clipNum = FPRandom.Range(2, 6);
                if (myMoveSetScript.AnimationExists("idle_" + clipNum) && !myMoveSetScript.IsAnimationPlaying("idle_"+ clipNum)) {
            ...
            if (inputRef.inputType == InputType.Button && !UFE.config.lockInputs)
            {
				...
				if (ev.button) {
					if (UFE.config.blockOptions.blockType != BlockType.HoldBack
                        ...
                        && myMoveSetScript.CompareBlockButtons(inputRef.engineRelatedButton) 
					    && currentSubState != SubStates.Stunned 
					    && !myPhysicsScript.isTakingOff
					    && !blockStunned
                        && myMoveSetScript.basicMoves.blockEnabled) {
	...
	public void SetMove(MoveInfo move)
    {
        ...
        if (currentMove != null)
        {
            ...
            if (move.animMap.clip != null && !myMoveSetScript.AnimationExists(move.name)) {
                Debug.LogWarning("Animation for move '" + move.name + "' not found! Make sure this move is assigned to the character's move set under Character Editor -> Move Sets");
            }
